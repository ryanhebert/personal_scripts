...
# Load user's personal overrides if they exist
...
_preserve_ssh_agent() {
    # When reattaching to screen, SSH_AUTH_SOCK from the original session is stale.
    # Fix: symlink to a stable path and point screen sessions at it.
...
uninstall_from_shell_rc() {
    ...
    # Remove the block: blank line before, the comment, the if block
    sed -i.bak '/# \[SSH ONBOARDING\]/,/^fi$/d' "$RC_FILE"
    ...
    echo ""
    printf "  ${C_YELLOW}${SYM_ARROW}${C_RESET}  ${C_WHITE}Also remove venv and workspace? ${C_DIM}[y/N]${C_RESET} "
    read -rsn1 answer
    echo ""
    ...
    if [[ "$answer" == "y" || "$answer" == "Y" ]]; then
        if [[ -d "$VENV_DIR" ]]; then
            rm -rf "$VENV_DIR"
            _step_ok "Removed ${C_CYAN}${VENV_DIR}${C_RESET}"
        fi
...
ensure_dependencies() {
    ...
    if [[ "$NEED_SCREEN" -eq 0 && "$NEED_PYTHON" -eq 0 ]]; then
        _step_ok "Dependencies satisfied ${C_DIM}(screen, python3)${C_RESET}"
        return 0
    fi
    ...
    # Define allowed packages explicitly (restrict sudo scope)
    local PACKAGES=()
    ...
    # Verify
    if command -v screen &>/dev/null && command -v python3 &>/dev/null; then
...
_install_requirements() {
    ...
    # Skip if hash matches (already installed this exact requirements.txt)
...
# Case A: Outside Screen — offer to attach
if [[ -z "$STY" ]]; then
    _check_root || return
    ...
    if _countdown "$ATTACH_TIMEOUT"; then
        ...
        _step_dim  "Session status:      ${C_CYAN}./onboard.sh --status${C_RESET}"
        _hr
        echo ""
    else
        # Timeout — auto-attach
...
if [[ -n "$STY" && "$STY" == *"$SCREEN_NAME"* ]]; then
    ...
    # Lock to prevent parallel setup races
    _acquire_lock
